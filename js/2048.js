...
//随机填充数字2或者4
  var predata,afterdata;
  var initNumber = function (num,iskeydown){
    predata = arrNumber.toString();//1.生成随机数之前的数据转换成字符串存给一个变量,好跟生成随机数字之后的数据进行比较;(因为数组时对象在改变数组的时候会把引用里的东西修改了,随之所有引用数组的地方的值也会改变，很难拿到改变之前的数据)
    var spaceArr = [];
    ...
    var isadd = iskeydown? ((spaceArr.length !== 0) && (predata !== afterdata)) :spaceArr.length !== 0;//3.不是用户操作(键盘和touch)初始化的时候只要有空格就生成随机数字；当时用户操作(键盘和touch)时,判断是否有空格并且当前上一个数据不等于这个数据的时候再添加随机数
    if(isadd){//有空格就继续生成随机数字
    ...
    afterdata = arrNumber.toString();//2.将添加随机数字之后的数据也转换成字符串进行存储
...
function redraw(){
  for (let i = 0; i < 4; i++) {
    for(let j = 0; j < 4; j++){
      ...
      var size = itemtext > 512? 2.124:3.214;//大于512改变字体大小为30
      var size = itemtext > 8192? 1.428:null;
      var weight = itemtext > 512? 600:400;//大于512改变粗体
      for (let k = 0; k < bagCor.length; k++){
        if (itemtext == Math.pow(2,k)) {
          item.css({'backgroundColor':bagCor[k],'fontSize':size + 'em','fontWeight':weight})
        }
      }
    }
  }
...
{//键盘事件
  ...
  var playRedraw = function (iskeydown){
    if (true) {
      var rannum = Math.random()*10 > 8? 4:2;
      initNumber(rannum,iskeydown);//点击一次添加一个数字
    }
    redraw();//重绘
...
$('#restart').click(
  function(){
    ...
    $('#nowScore').text(0);
    score = 0;
    $('#table2048').removeClass('table2048');
  }
)